...
import { sendMessageStream, ToolEvent, healthCheck } from '../services/apiService';
...
const ChatPage: React.FC = () => {
  ...
  const [agentActivities, setAgentActivities] = useState<ToolEvent[]>([]);
    ...
    const checkBackend = async () => {
      try {
        ...
        setMessages([{
          id: 'init-1',
          role: 'model',
          text: `🦜 **KeaBOT está pronto!**\n\nBackend conectado. Provider: ${llmProvider.toUpperCase()}\n\nSou seu agente de automação local. Posso navegar arquivos, buscar código e executar skills. Como posso ajudar?`,
          timestamp: new Date()
        }]);
      } catch {
  ...
  const handleSendMessage = useCallback(async () => {
    if (!inputValue.trim() || isLoading || backendStatus !== 'online') return;
    ...
    try {
      ...
      setMessages(prev => prev.map(msg =>
        msg.id === modelMsgId
          ? { ...msg, text: "Desculpe, ocorreu um erro ao processar sua mensagem." }
          : msg
      ));
      setIsAgentActive(false);
    } finally {
  ...
  }, [inputValue, isLoading, backendStatus, sessionId, llmProvider]);
  ...
  return (
    <main className="flex-1 flex flex-col h-full relative bg-[#101922]">
        ...
        <div className="flex items-center gap-3 text-white">
          ...
          <div>
            ...
            <p className="text-xs text-[#92adc9]">
              {llmProvider === 'gemini' ? 'Gemini Flash' : 'OpenAI GPT-4'} • Backend Python
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {/* LLM Provider Toggle */}
          <div className="flex items-center gap-1 bg-[#1c2a38] rounded-lg p-1">
            <button
      ...
      {(isAgentActive || agentActivities.length > 0) && (
        <AgentActivity activities={agentActivities} isActive={isAgentActive} />
      )}
        ...
        <div className="max-w-[800px] mx-auto pointer-events-auto">
          <div className="relative bg-[#1c2a38]/80 backdrop-blur-xl border border-[#233648] rounded-xl shadow-2xl flex flex-col gap-2 p-3 transition-all focus-within:ring-2 focus-within:ring-primary/50 focus-within:border-primary/50">
            <textarea
              ...
              placeholder={backendStatus === 'online' ? "Ask KeaBOT anything..." : "Backend offline..."}
            ...
            <div className="flex items-center justify-between px-2 pb-1">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  disabled={isLoading || !inputValue.trim() || backendStatus !== 'online'}
                  className={`size-9 flex items-center justify-center rounded-lg bg-primary hover:bg-blue-600 text-white shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95 ${isLoading || !inputValue.trim() || backendStatus !== 'online' ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
